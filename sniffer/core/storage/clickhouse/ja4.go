...
func (c *ClickHouseStorage) LookupJA4(ctx context.Context, fp string) (*models.Ja4Entry, error) {
    ...
    SELECT id, fingerprint, application, library, device, os, observation_count, verified, updated_at
    FROM ja4_database WHERE fingerprint = ? LIMIT 1
`, fp).Scan(&e.ID, &e.Fingerprint, &e.Application, &lib, &dev, &e.OS, &e.ObservationCount, &e.Verified, &updatedAt)
